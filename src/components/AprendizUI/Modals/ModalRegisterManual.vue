<template>
  <BaseModal @close="close" ref="modalRef" :title="mode === 'salidas' ? 'Registro manual de salida' : 'Registro manual de ingreso'">
    <div class="rounded-2xl bg-slate-50 p-4">
      <p class="mb-1 text-sm font-semibold text-slate-700">
        {{ mode === 'salidas' ? 'Complete el documento y valide la identidad antes de registrar la salida.' : 'Complete el documento y valide la identidad antes de registrar el acceso.' }}
      </p>
    ...
    <ManualEntryForm :mode="mode" @submit-manual="handleSubmit"/>
  </BaseModal>
</template>
